...
#define ESDF_VOXEL_CHAR_SIZE 64

inline size_t writeEsdftoBuffer(const Layer<EsdfVoxel>& layer, char** buffer){
    BlockIndexList blocks;
    ...
    size_t buf_size = num_voxels_per_block * blocks.size() * ESDF_VOXEL_CHAR_SIZE;

    *buffer = (char*)malloc(buf_size);
    size_t offset = 0;

    offset += sprintf(*buffer+offset, "x,y,z,distance,parent_x,parent_y,parent_z,observed\n");
    ...
    for (const BlockIndex& index : blocks) {
        ...
        for (size_t linear_index = 0; linear_index < num_voxels_per_block; ++linear_index) {
            Point coord = block.computeCoordinatesFromLinearIndex(linear_index);
            EsdfVoxel curr_voxel = block.getVoxelByLinearIndex(linear_index);

            offset += sprintf(*buffer+offset, "%f,%f,%f,%f,%d,%d,%d,%d\n", (double)coord.x(), (double)coord.y(), (double)coord.z(), (double)curr_voxel.distance, curr_voxel.parent[0], curr_voxel.parent[1], curr_voxel.parent[2], curr_voxel.observed);
        }
    }
