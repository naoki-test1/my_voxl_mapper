...
namespace loco_planner
{
      ...
      double total_time, mav_trajectory_generation::Vertex::Vector *vertices)
  {
    ...
    poly_opt_.setupFromVertices(*vertices, times, mav_trajectory_generation::kDerivativeToOptimize);
    ...
    if (config_.soft_goal_constraint)
    {
      ...
      poly_opt_.setupFromVertices(*vertices, times,
                                  mav_trajectory_generation::kDerivativeToOptimize);
      ...
      size_t num_segments)
  {
    ...
    std::vector<double> times(num_segments, total_time / (double)num_segments);
    mav_trajectory_generation::Vertex::Vector vertices;
    vertices.reserve(times.size() + 1);
                                              ...
                                              double *gradient) const
  {
    if (parent_ == nullptr)
    {
      fprintf(stderr, "loco pointer is null: %s\n", __FUNCTION__);
      return false;
    }
