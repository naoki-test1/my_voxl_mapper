...
class PolynomialOptimization {
 ...
 public:
  ...
  PolynomialOptimization(size_t dimension, int derivative_to_optimize, size_t n_segments);
  ...
  void getTrajectory(Trajectory* trajectory) const {
    if(trajectory == nullptr){
        fprintf(stderr, "trajectory is nullptr: %s\n", __FUNCTION__);
        return;
    }
  ...
  void getVertices(Vertex::Vector* vertices) const {
    if(vertices == nullptr){
        fprintf(stderr, "vertices is nullptr: %s\n", __FUNCTION__);
        return;
    }
    *vertices = vertices_;
  }
  ...
  void getSegments(Segment::Vector* segments) const {
    if(segments == nullptr){
        fprintf(stderr, "segments is nullptr: %s\n", __FUNCTION__);
        return;
    }
    *segments = segments_;
  }
  ...
  void getSegmentTimes(std::vector<double>* segment_times) const {
    if(segment_times == nullptr){
        fprintf(stderr, "segment_times is nullptr: %s\n", __FUNCTION__);
        return;
    }
  ...
  void getFixedConstraints(
    ...
    if(fixed_constraints == nullptr){
        fprintf(stderr, "fixed_constraints is nullptr: %s\n", __FUNCTION__);
        return;
    }
...
#include "polynomial_optimization_linear_impl.h"
