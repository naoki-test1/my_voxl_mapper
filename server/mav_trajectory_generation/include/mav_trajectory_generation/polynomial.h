...
class Polynomial {
 public:
  ...
  Polynomial(int N, const Eigen::VectorXd& coeffs)
    ...
    if(N_ != coeffs.size()){
        std::cout << "Number of coefficients has to match." << std::endl;
    }
  }
  ...
  Eigen::VectorXd getCoefficients(int derivative = 0) const {
    if(derivative > N_){
        throw std::runtime_error(std::string("derivative is larger than N"));
    }
    if (derivative == 0) {
  ...
  void evaluate(double t, Eigen::VectorXd* result) const {
    if(result->size() > N_){
        fprintf(stderr, "result is larger than N: %s\n", __FUNCTION__);
        return;
    }
  ...
  static void baseCoeffsWithTime(int N, int derivative, double t,
    ...
    if(derivative > N){
        fprintf(stderr, "derivative is larger than N: %s\n", __FUNCTION__);
        return;
    }
    if(derivative < 0){
        fprintf(stderr, "derivative is < 0: %s\n", __FUNCTION__);
        return;
    }
