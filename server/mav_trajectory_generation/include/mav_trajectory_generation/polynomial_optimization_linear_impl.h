...
bool PolynomialOptimization<_N>::setupFromVertices(
    ...
    if (!(derivative_to_optimize >= 0 && derivative_to_optimize <= kHighestDerivativeToOptimize)){
        std::cout << "You tried to optimize the " << derivative_to_optimize
        << "th derivative of position on a " << N
        << "th order polynomial. This is not possible, you either need a higher "
           "order polynomial or a smaller derivative to optimize." << std::endl;
        return false;
    }
    ...
    if (n_vertices_ != times.size() + 1){
        std::cout << "Size of times must be one less than positions." << std::endl;
    }
    ...
    for (size_t vertex_idx = 0; vertex_idx < n_vertices_; ++vertex_idx) {
        ...
        for (Vertex::Constraints::const_iterator it = vertex.cBegin();
            ...
            if (it->first > kHighestDerivativeToOptimize) {
                vertex_valid = false;
                std::cout << "Invalid constraint on vertex " << vertex_idx
                             << ": maximum possible derivative is "
                             << kHighestDerivativeToOptimize << ", but was set to "
                             << it->first << ". Ignoring constraint" << std::endl;
            } else {
...
double PolynomialOptimization<_N>::computeCost() const {
    if(!(n_segments_ == segments_.size() && n_segments_ == cost_matrices_.size())){
        fprintf(stderr, "n_segments is not correct: %s\n", __FUNCTION__);
        return -1;
    }
    double cost = 0;
...
void PolynomialOptimization<_N>::updateSegmentTimes(
    ...
    if(n_segment_times != n_segments_){
        std::cout << "Number of segment times (" << n_segment_times
        << ") does not match number of segments (" << n_segments_ << ")" << std::endl;
        return;
    }
    ...
    for (size_t i = 0; i < n_segments_; ++i) {
        ...
        if(segment_time <= 0){
            std::cout << "Segment times need to be greater than zero" << __FUNCTION__ << std::endl;
            continue;
        }
...
void PolynomialOptimization<_N>::constructR(
    ...
    if(R == nullptr){
        fprintf(stderr, "R is a nullptr: %s\n", __FUNCTION__);
        return;
    }
    typedef Eigen::Triplet<double> Triplet;
...
bool PolynomialOptimization<_N>::solveLinear() {
    if(!(derivative_to_optimize_ >= 0 && derivative_to_optimize_ <= kHighestDerivativeToOptimize)){
        fprintf(stderr, "derivative to optimize is not in bounds: %s\n", __FUNCTION__);
        return false;
    }
...
bool PolynomialOptimization<_N>::computeSegmentMaximumMagnitudeCandidates(
    ...
    if(N - derivative - 1 <= 0){
        std::cout << "N-Derivative-1 has to be greater 0" << std::endl;
        return false;
    }
...
void PolynomialOptimization<_N>::setFreeConstraints(
    ...
    if(free_constraints.size() != dimension_){
        fprintf(stderr, "free constraints size is wrong: %s\n", __FUNCTION__);
        return;
    }
    for (const Eigen::VectorXd& v : free_constraints)
        if(static_cast<size_t>(v.size()) != n_free_constraints_){
            fprintf(stderr, "n free constraints size is wrong: %s\n", __FUNCTION__);
            return;
        }
...
void PolynomialOptimization<_N>::getAInverse(Eigen::MatrixXd* A_inv) const {
    if(A_inv == nullptr){
        fprintf(stderr, "A inv is a nullptr: %s\n", __FUNCTION__);
        return;
    }
...
void PolynomialOptimization<_N>::getM(Eigen::MatrixXd* M) const {
    if(M == nullptr){
        fprintf(stderr, "M is a nullptr: %s\n", __FUNCTION__);
        return;
    }
...
void PolynomialOptimization<_N>::getA(Eigen::MatrixXd* A) const {
    if(A == nullptr){
        fprintf(stderr, "A is a nullptr: %s\n", __FUNCTION__);
        return;
    }
...
void PolynomialOptimization<_N>::getMpinv(Eigen::MatrixXd* M_pinv) const {
    if(M_pinv == nullptr){
        fprintf(stderr, "Mpinv is a nullptr: %s\n", __FUNCTION__);
        return;
    }
...
void PolynomialOptimization<_N>::computeQuadraticCostJacobian(
    ...
    if(derivative >= N){
        fprintf(stderr, "derivative is too large: %s\n", __FUNCTION__);
        return;
    }
