...
inline Eigen::Vector3d vector3FromMsg(const Vector3& msg) {
  return Eigen::Vector3d(msg.x, msg.y, msg.z);
}

inline Eigen::Quaterniond quaternionFromMsg(const Quaternion& msg) {
...
inline bool vectorFromRotationMatrix(const Eigen::Matrix3d& mat,
  ...
  double cos_phi_clamped = std::max(-1.0, std::min(cos_phi, 1.0));
    ...
    const Eigen::Vector3d& rot_vec_acc)
{
  ...
  W_vel = phi_skew * (phi * std::sin(phi) - 2.0 + 2.0 * std::cos(phi)) * phi_dot *
              phi_3_inv +
          phi_skew * phi_skew *
              (-2.0 * phi - phi * std::cos(phi) + 3.0 * std::sin(phi)) * phi_dot *
              phi_4_inv +
          phi_dot_skew * phi_skew * (phi - std::sin(phi)) * phi_3_inv;

  W_acc = Eigen::MatrixXd::Identity(3, 3) +
          phi_skew * (1.0 - std::cos(phi)) * phi_2_inv +
          phi_skew * phi_skew * (phi - std::sin(phi)) * phi_3_inv;
